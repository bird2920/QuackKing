...
export default function PlayerGameScreen({ db, gameCode, lobbyState, players, currentQuestion, userId }) {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 text-white px-4 py-6 sm:py-10">
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col items-center space-y-6 sm:space-y-8 text-center">
        <div className="space-y-1.5">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-black">
            {player.name}
            <span className="ml-2 text-sm sm:text-base font-semibold text-slate-300">
              • {player.score} pts
            </span>
            <div className="w-full my-1.5 border-t border-white/10">
              {hasTheme && (
                <div className="inline-flex items-center gap-2 mt-3 rounded-2xl border border-white/20 bg-white/5 px-3 py-1 text-xs sm:text-sm uppercase tracking-[0.25em] text-slate-300">
        ...
        <div className="rounded-3xl border border-white/10 bg-white/5 p-4 sm:p-6 backdrop-blur-2xl shadow-2xl shadow-indigo-950/40 w-full">
          {startCountdown > 0 && (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-[0.8rem] sm:text-sm uppercase tracking-[0.35em] text-slate-400">
        ...
        <div className={`w-full rounded-3xl border border-white/5 bg-white/5 p-4 sm:p-6 backdrop-blur-2xl shadow-[0_25px_120px_-35px_rgba(99,102,241,0.4)] transition-transform duration-150 ${shouldBounce ? 'animate-bounce-hint' : ''}`}>
          <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
            {currentQuestion.options.map((option, i) => {
              ...
              if (isLocked && answerRevealed) {
                ...
                optionClasses = "bg-gradient-to-r from-indigo-500 to-blue-500 text-white border-transparent shadow-indigo-900/40";
              }
        ...
        {answerRevealed && (
          <div className="w-full rounded-3xl border border-white/10 bg-slate-900/60 p-3 sm:p-4 backdrop-blur-xl shadow-xl max-h-[280px] overflow-y-auto flex flex-col space-y-1.5 sm:space-y-2">
            <p className="text-[0.7rem] sm:text-xs uppercase tracking-[0.2em] text-slate-400 mb-1.5 sm:mb-2">
              Leaderboard
            </p>
            ...
            {sortedPlayers.map((p, i) => (
              <div
                key={p.id}
                className={`flex justify-between items-center text-sm px-3 py-2 rounded-xl ${p.id === userId
        ...
        <p className="text-xs sm:text-sm text-slate-500">Game Code: {gameCode}</p>
      </div>
    </div>
  );
}
